...
2. **Research ToDo adoption (PRD-C15-001 §9.3):** After reading the cycle objective and any active idea notes, query pending project ToDos via `hero adopt-todo` / SQLite (the Hero TUI injects the authoritative pending list). Present concise IDs and summaries in the user's chat language. Ask which remaining items, if any, should enter this cycle. Persist adopted selections with `hero adopt-todo <id>...` (optional `--note`) — do **not** edit OpenSpec checkboxes or finding state yourself. If adoption persistence fails, retry before finalizing requirements. Include adopted items in requirements handed to Planning.

3. **Active idea notes (optional):** At Research session start, run `hero cycle idea-files` (or list `docs/idea/` manually, excluding top-level `archive/` and `tobe/`). If any paths are returned, read each file before grilling. Treat them as **non-normative** input only — cycle PRD/ADR/UI supersede on conflict (ADR-019). Skip this step when the list is empty.
4. Conduct a structured grilling session with the user:
...
5. Summarize the agreed requirements for confirmation.
6. **Pre-document gate (mandatory):** Before generating any documents, ask the user whether they want to add any more information about the project. Wait for their reply in this turn — do **not** create documents in the same message as this question.
7. If the user adds information: evaluate impact (scope, constraints, trade-offs, which docs are needed); incorporate into the agreed requirements; if the additions open material ambiguity, ask short follow-up questions before generating.
8. Only after the gate (and any follow-ups) is resolved: decide which documents to create (PRD, ADR, UI, DEPLOY, TESTING) and register them in `documents.json`.
9. Generate the decided documents in the appropriate `docs/` subdirectory.
10. Number cycle documents: `[CATEGORY]-C[XX]-[seq]-[slug].md`.
11. Write all documents in English regardless of chat language.
12. Report completion to the orchestrator with a summary and document list.
