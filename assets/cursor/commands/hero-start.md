...
5. For each enabled stage, invoke the appropriate agent via the Task tool in a fresh isolated session.
...
If the configured model is unavailable, fall back to `generic_model` and warn the user explicitly every time fallback activates. If still unavailable, warn and wait for /hero:continue.
