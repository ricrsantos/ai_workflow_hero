...
## 2026-09-25 — Model catalog refresh: GPT-6 Sol/Luna, Claude Opus 5.5, Grok 4.7, MiMo V2.6

**Pedido**: usuário pediu para atualizar `assets/models/*.yml` (e espelhar em `.workflow-hero/models/`) com modelos lançados recentemente para os 4 harnesses: Codex (GPT-6 Sol/Luna), OpenCode (MiMo 2.6), Claude (Opus 5.5), Cursor (Grok 4.7).

**Pesquisa**: dados de pricing/context-window obtidos via `WebSearch` (não inventados — instrução explícita do AGENTS.md e do usuário), com fontes: OpenAI/VentureBeat (GPT-6 Sol $2/$10, Luna $0.10/$0.50, cached 10% do input, contexto 1.05M, lançados 2026-09-22); Claude Platform Docs/Claude Code release notes (Opus 5.5 $4/$20, cache_write $5 no cache de 5min, contexto 1M, vira o alias `opus` padrão no Claude Code ≥2.1.280); Cursor Docs/x.ai (Grok 4.7 preço idêntico ao 4.6 — $2/$6, cached $0.50, contexto 500K — mas effort padrão no Cursor mudou de `medium` para `high`); OpenRouter (MiMo V2.6 Flash/Pro/Pro-UltraSpeed, contexto 1.05M, e novidade: toggle de reasoning on/off que a V2.5 não tinha).

**Mudanças**: `assets/models/{codex,openai,claude,anthropic,xai,cursor,opencode}.yml` — modelos novos adicionados seguindo os padrões já existentes em cada arquivo (ex.: `cache_write = 1.25× input` para a família GPT, variantes `-low/-medium/-high/-xhigh(-fast)` para Cursor Grok, linhas duplicadas `provider/model` para OpenCode-native). `.workflow-hero/models/*.yml` copiado 1:1 de `assets/models/*.yml` — é o overlay que `hero update-models` normalmente busca do `main` deste mesmo repo, então aqui os dois diretórios são mantidos sincronizados manualmente. Testes ajustados: `internal/modelprops/catalog_test.go` (lista exata `ModelsForHarness("claude")` + `wantWindow` ganharam `claude-opus-5-5`), `internal/modelprops/codex_catalog_test.go` (cobertura de existência para `gpt-6-sol`/`gpt-6-luna`).

**Decisão de escopo não confirmada com o usuário**: o `gpt-6-sol` foi adicionado só em `codex.yml` (não em `openai.yml`/`opencode.yml`), espelhando o precedente já existente do `gpt-5.6-sol` (tier exclusivo ChatGPT Plus/Pro), mesmo a busca indicando que a API OpenAI também expõe `gpt-6-sol` diretamente — decisão foi manter consistência com o padrão do arquivo em vez de divergir com base numa única fonte. Também identificado o **GPT-6 Astra** (lançado 2026-09-03, $10/$50, contexto 1.05M) como um terceiro modelo da mesma geração que o usuário não pediu explicitamente — perguntado ao usuário, que confirmou a inclusão.
