...
- **HarnessAdapter (full)**: Shared `StreamDelta` + TUI warn-only watchdog (Cursor 5m / OpenCode+Codex 6m). **Cursor** NDJSON; **OpenCode** SSE + serve lifecycle + `PrepareHeroStart`; **Codex** (`internal/adapters/codex`) Hero-managed `codex app-server` stdio JSON-RPC — lazy start, SQLite registry (no URL), stream/permission/auth/`ListModels`/C5 map, orphan reap, `ResetAppServer` + `PrepareHeroStart`. Agent text: raw deltas + authoritative `item/completed` (no duplicate); reasoning → thinking on completed only; noisy activities / unrecognized warnings only with `hero --debug`. `harnessmgr` resolves cursor+opencode+codex; fallback chain ADR-033; no cross-harness session resume.
