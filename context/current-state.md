...
- Chat is first in the TUI tab bar (`alt+1`) and is the boot screen. Chat `/` overlay lists Go to / palette items; only `/hero-approve` `/hero-reject` `/hero-cancel` `/hero-continue` `/hero-finish` `/hero-back` insert into the composer — other items execute immediately like the full-screen palette. Chat header shows a live agents box; transcript labels parent vs Task subagents (2026-08-14).
