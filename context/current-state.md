...
- **2026-08-28 — Shared TUI timers (ADR-058)**: Replaced the `/hero-start`-owned footer elapsed value with one-second `internal/tui/timers.go` ticks. The blue bottom-navbar `Sessão` timer starts at `/hero-new`, persists active cycle seconds in SQLite, stops at cycle completion, and resets at archive/free-chat; free-chat Session starts on its first prompt and is process-local. The blue `AI` timer starts per Execute, stops at the last response/cancel, and is never persisted. All values format as continuous `HH:MM:SS`; pending `/hero-new` state ignores stale previous-cycle refreshes and clears on cancellation.
