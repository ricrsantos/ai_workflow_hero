...
| **Phase** | Hero **2.9.0** released (tag `v2.9.0`). TUI settings screen, checklist window, harness config adjustments, auto-hide Config after archive, label/status-bar polish. |
...
- `scripts/release.sh` + `build_dev.sh` + contract tests; latest release **2.9.0**; integration tests include C6 Codex path.
