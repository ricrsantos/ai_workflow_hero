...
- **HarnessAdapter (full)**: Cursor adapter unchanged Execute/stream/cancel semantics; **OpenCode adapter** (`opencode serve`, HTTP Execute/Cancel, `ListModels`, **normal shutdown** on TUI quit and `/hero-harness` disable); **`harnessmgr.Registry`** resolves by id; TUI Execute routes via **two-step** fallback chain (agent pair → `fallback_model` → hard stop; no freechat third fallback) and persists `stages.harness_id`; **session binding** never resumes a Cursor session as OpenCode.
...
- **`/hero-harness`** enable/disable with OpenCode projection on enable; **`/hero-model`** two-column pair picker; Chat labels **`[LABEL - model · harness]`**; OpenCode HTTP adapter tests; `assets/opencode/` embedded in binary.
