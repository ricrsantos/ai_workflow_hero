...
- **Telegram plugin (C9, landed)**: optional official plugin — `hero plugin install|uninstall|list telegram` copies a platform-matched `hero-telegram-daemon` and records `manifest.json` (`internal/plugin`). Per-OS-user daemon (`internal/telegram/daemon`) owns Bot API + versioned `0600`-socket IPC (`internal/telegram/ipc`) + OS-vault credentials (`internal/telegram/vault`), allocates `base`/`_2`/`free_N` suffixes, routes addressed inbound, durably queues offline targets (24h), and sends lifecycle-only outbound. TUI (`internal/tui/telegram*.go`) runs the IPC client as `tea.Cmd` with bounded-backoff reconnect, Settings Telegram section + pairing modal, and `←/→ [Telegram · addr]` transcript labels; every harness turn reaches its adapter through the shared transport-neutral `internal/conversation.Service`. Project logs rotate under `.workflow-hero/logs/tui.log`; daemon logs under `~/.workflow-hero/logs/telegram-daemon.log` (10 MB × 10, redacted). Doctor/status report plugin health.
