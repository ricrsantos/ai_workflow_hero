...
| **Phase** | Hero **2.1.1** released 2026-08-18 (OpenCode stream fix); **C5 archived** 2026-08-18 (`model-properties-tui`). **C4 archived** 2026-08-16 (`hero-2-0-multi-harness`). No active cycle. |
...
- **HarnessAdapter (full)**: Shared **`StreamDelta` normalization** (`internal/harness/stream.go`: text/thinking/tool/warning/permission/activity/session); Cursor NDJSON parser warns on unknown types and fails on exit-without-`result`; **OpenCode adapter** SSE map aligned with `anomalyco/opencode` `dev` `EventManifest.Definitions` (`events.go`: v1 `message.*` + v2 `session.next.*` streaming, `permission.v2.asked`, `sync` unwrap, transport events); `permission.asked`/`permission.v2.asked` → TUI `Allow? [y/N]` + `POST /permission/{id}/reply`; serve URL from child stdout (ADR-035); **`harnessmgr.Registry`** per harness id; two-step fallback chain; session binding never mixes Cursor/OpenCode sessions.
...
- Model catalog pricing/capabilities: `assets/models/*.yml` (mirrored in `.workflow-hero/models/`) carry 2026-08-18 researched pricing (`input`/`cache_write`/`cache_read`/`output`/`context_window`) plus C5 `properties` (`fs`/`ef`/`th`) for anthropic/google/moonshot/openai/xai/zhipu/cursor providers; opencode-go rows got pricing (properties kept as-is). Unsupported properties use `values: ["na"]`/`default: "na"`; unfindable data uses `["not found"]`. Cursor Grok context windows corrected to 500000 (grok-4.5/4.6), GLM-5.2 and Kimi K3 to 1M, fast Grok variants priced at $4/$1/$12.
