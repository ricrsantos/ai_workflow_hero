...
- **Feature Based + Vertical Slice**: `cmd/hero` + `internal/<feature>/` (`install`, `upgrade`, `uninstall`, `doctor`, `status`, `variables`, `update_models`, `cycle`, `store`, `engine`, `tui`, `harness`, `harnessmgr`, `todos`, `workflowconfig`) + `internal/adapters/cursor/` + `internal/adapters/opencode/` + `internal/common/` (includes `assetconflict` for upgrade conflict backup/replace).
...
- Upgrade: checksum-based conflict detection; customized files are backed up as `{filename}_{timestamp}.conflict` and replaced with the new version (warning per file + summary); reconciles stale `checksums.json` when disk already matches embedded assets; also ensures env hygiene files/patterns; refreshes `docs/workflow-help.md` when not customized.
...
- `update-models`: fetches structured upstream model YAML (HTTP client injectable for tests); same conflict backup/replace behavior as upgrade for customized `.workflow-hero/models/*.yml`; updates `checksums.json`.
