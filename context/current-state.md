...
| **Phase** | Hero **1.0.5** patch release; **C3 archived** (2026-08-09). No active cycle. |
...
- **HarnessAdapter (full)**: `IsAvailable`, sessions, `Execute`/`Cancel`/`Status`, `Dispatch`→Execute; Cursor adapter runs Agent CLI (`cursor-agent` / `cursor agent`) with json/stream-json parsers; stream mode uses `--stream-partial-output` + `StreamingCommandRunner` (live stdout pipe → `ParseStreamJSON`); `OnStreamDelta` emits text, thinking, and tool activity; injectable `CommandRunner`. Execute passes `--trust --force --workspace <projectDir>` so non-interactive TUI runs can use Shell (`hero` CLI) and stay in the consumer project. `Cancel("")` aborts in-flight Execute before a Cursor session id exists (TUI Ctrl+C during `/hero-start`). Execute retries `RetriableError` / `resource_exhausted` (auth/trust fail fast).
