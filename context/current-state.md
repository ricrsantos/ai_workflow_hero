...
| **Phase** | Hero **3.0.1** released (tag `v3.0.1`). Patch: `hero plugin install telegram` downloads the daemon from GitHub Releases (matching Hero version + platform). |
...
- `scripts/release.sh` + `build_dev.sh` + contract tests; latest release **3.0.1** (ships `hero-telegram-daemon` per platform; plugin install downloads from GitHub Releases); integration tests include C6 Codex path and C9 Telegram lock.
...
1. C9 QA closed after `/hero-continue 2` using the passing TUI re-run (`go test ./...`, build, vet, logging). Judge is Running 2/3 (TUI `judge_agent`). Browser UI Validation and QA End-to-End remain skipped.
2. C7/C8 QA and Judge; archive OpenSpec changes when approved.
3. C6 archive (`codex-adapter`) when ready.
