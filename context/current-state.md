...
| **Phase** | Hero **2.8.0** released (tag `v2.8.0`). TUI cycle Config (C7), TUI-direct stage Execute (C8), shared Session/AI timers; harness permission profiles. |
...
- `scripts/release.sh` + `build_dev.sh` + contract tests; latest release **2.8.0**; integration tests include C6 Codex path.
...
- **2026-08-29 — Unified Harness Manager permissions**: `/harness` now presents each Harness and its indented permission choices in one screen. Disabled Harnesses keep their choices visible but inert and retain their persisted profile. Space toggles a focused Harness or profile; Enter persists the topology and enabled-Harness profiles together. No choice (or an invalid choice) resolves conservatively to `Ask every time`; if both choices are selected, `Automatic in project` wins. The former two-step permission picker was removed.
