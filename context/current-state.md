...
- **HarnessAdapter (full)**: Cursor adapter unchanged Execute/stream/cancel semantics; **OpenCode adapter** (`opencode serve`, HTTP Execute/Cancel, `ListModels`, **normal shutdown** on TUI quit and `/hero-harness` disable); serve URL parsed from child stdout (ephemeral port, ADR-035); streaming uses OpenCode SSE `/event` + `{providerID,modelID}` model payload; **`harnessmgr.Registry`** resolves by id; TUI Execute routes via **two-step** fallback chain (agent pair → `fallback_model` → hard stop; no freechat third fallback) and persists `stages.harness_id`; **session binding** never resumes a Cursor session as OpenCode.
