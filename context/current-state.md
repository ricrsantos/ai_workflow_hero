...
- **Bubble Tea TUI** (`hero tui`): Left nav sidebar with live **agents** block (named 4-letter codes plus nested generic `TASK` chips), a bottom blue `Sessão`/`AI` timer subdivision, and Chat/Status/Artifacts/Costs/Events, with Config appended only for an active cycle. Orchestrator Execute uses `agents.orchestration_agent`. `/hero-start` tells ORCH to `hero stage start` then STOP; `/hero-approve` does the same for the next Waiting stage (no second `/hero-start`). The TUI Executes named stage agents on their YAML harness+model pair (C8; Research still uses dedicated `discover_agent`). Implementation may run BACK/FRNT/GEN concurrently; nested Task fan-out stays in the parent harness. Chat multiplexes tagged Executes, shows `[LABEL - model · harness]` including Task start lines, keeps `Waiting for harness…` while any Execute is live, and Esc cancels all in-flight Executes; Ctrl+C remains the protected quit shortcut while an agent is running. Numbered navigation follows the visible list: `alt+1-5` without Config and `alt+1-6` with Config; hidden `alt+6` is a no-op. `/hero-start` validates/syncs YAML asynchronously; OpenCode/Codex Prepare resets serve/app-server. Control slashes stay on orchestrator. Composer is Build/Plan Tab; auto-follow scroll; context bar from `result.usage`. Session starts at zero on TUI boot; `/hero-start` and `/hero-resume` explicitly restore persisted cycle seconds, while `/hero-new` starts at zero. Free-chat Session starts on the first prompt and continues across prompts until a cycle starts or the TUI exits; free-chat Session and AI timers are process-local.
...
- **2026-08-28 — Shared TUI timers (ADR-058)**: Replaced the `/hero-start`-owned footer elapsed value with one-second `internal/tui/timers.go` ticks. The blue bottom-navbar `Sessão` timer starts at zero on TUI boot, restores persisted cycle seconds only for explicit `/hero-start` or `/hero-resume`, starts new cycles at zero, stops at cycle completion, and resets at archive or when entering free chat; free-chat Session starts on its first prompt, continues across prompts, and is process-local. The blue `AI` timer starts per Execute, stops at the last response/cancel, and is never persisted. All values format as continuous `HH:MM:SS`; pending `/hero-new` state ignores stale previous-cycle refreshes and clears on cancellation.

- **2026-08-27 — TUI interruption and conditional navigation**: Esc cancels active Harness Executes/preflight; Ctrl+C remains a protected quit shortcut. Sidebar order is Chat, Status, Artifacts, Costs, Events, Config; the visible shortcut label is `alt+1-5` without an active cycle and `alt+1-6` with Config, and hidden `alt+6` is ignored.
