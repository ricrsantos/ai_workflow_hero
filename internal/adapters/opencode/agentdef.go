...
// .opencode/agents/<agentName>.md frontmatter. OpenCode reads the model from
// the agent file when a subagent is launched; Hero must sync before /hero-start.
func SyncAgentDefinition(projectDir, agentName string, cfg workflowconfig.AgentModelConfig) error {
	...
	path := filepath.Join(install.OpenCodePathsFor(projectDir).Agents, agentName+".md")
