...
> **Status:** reflects codebase at Hero **3.0.4** (Config model catalog picker, welcome dialog fill, Config property/catalog cascade). Cursor + OpenCode + Codex TUI harnesses; Execute/Prepare/orphan/health wired. Optional Telegram plugin (C09) landed: per-OS-user daemon, versioned IPC, conversation service, OS-vault credentials, rotating logs, remote cycle-config wizard, and active-agent/model Telegram status reporting.
