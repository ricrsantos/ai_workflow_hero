...
> **C9 timer ADR:** [ADR-C09-001-tui-session-ai-timers.md](ADR-C09-001-tui-session-ai-timers.md) (ADR-058). **C9 Telegram ADR:** [ADR-C09-002-telegram-integration.md](ADR-C09-002-telegram-integration.md) (ADR-059–065). **C13 Claude Code ADRs:** [ADR-C13-001-claude-code-adapter.md](ADR-C13-001-claude-code-adapter.md) (ADR-070–074). **TUI completion-gate amendment:** [ADR-C08-001-tui-stage-execute.md](ADR-C08-001-tui-stage-execute.md#adr-075-implementation-completion-is-gated-by-structured-reports-and-the-openspec-checklist) (ADR-075). **C14 development auto-update ADR:** [ADR-C14-001-auto-update.md](ADR-C14-001-auto-update.md) (ADR-076). **C14 multimodal images ADRs:** [ADR-C14-002-tui-multimodal-images.md](ADR-C14-002-tui-multimodal-images.md) (ADR-077–082). **C15 findings and deferred ToDos ADRs:** [ADR-C15-001-loopback-findings-handoff.md](ADR-C15-001-loopback-findings-handoff.md) (ADR-083–090). **C16 durable session history ADRs:** [ADR-C16-001-tui-session-history.md](ADR-C16-001-tui-session-history.md) (ADR-091–098).
>
...
| [ADR-054](ADR-C08-001-tui-stage-execute.md#adr-054-tui-executes-named-stage-agents-on-their-yaml-pair) | TUI Executes named stage agents on their YAML pair | Accepted |
...
| [ADR-075](ADR-C08-001-tui-stage-execute.md#adr-075-implementation-completion-is-gated-by-structured-reports-and-the-openspec-checklist) | Implementation completion is gated by structured reports and the OpenSpec checklist | Accepted |
...
| [ADR-059](ADR-C09-002-telegram-integration.md#adr-059-telegram-ships-as-an-optional-official-plugin-with-a-local-daemon) | Optional official Telegram plugin with local daemon | Proposed |
| [ADR-060](ADR-C09-002-telegram-integration.md#adr-060-versioned-os-user-ipc-replaces-multi-tui-database-polling) | OS-user IPC replaces multi-TUI database polling | Proposed |
| [ADR-061](ADR-C09-002-telegram-integration.md#adr-061-conversation-core-is-shared-transport-adapters-stay-at-the-edge) | Shared conversation core with edge transport adapters | Proposed |
| [ADR-062](ADR-C09-002-telegram-integration.md#adr-062-pair-one-chat-and-isolate-all-telegram-credentials-in-the-os-vault) | One-chat pairing and OS vault credential isolation | Proposed |
...
| [ADR-077](ADR-C14-002-tui-multimodal-images.md#adr-077-common-multimodal-contract-lives-in-internalharness) | Common multimodal contract in `internal/harness`; promoted to `internal/media` only if justified | Accepted |
| [ADR-078](ADR-C14-002-tui-multimodal-images.md#adr-078-immutable-asset-references-in-bubble-tea-model--image-bytes-never-stored-in-model-state) | Immutable asset references in Bubble Tea model — image bytes never stored in model state | Accepted |
| [ADR-079](ADR-C14-002-tui-multimodal-images.md#adr-079-explicit-capability-failure--blocked-turn-with-actionable-error-when-model-does-not-support-images) | Explicit capability failure — blocked turn with actionable error when model does not support images | Accepted |
| [ADR-080](ADR-C14-002-tui-multimodal-images.md#adr-080-temporary-assets-stored-in-localsharehero-sessions-idassets-session-scoped-only) | Temporary assets in `~/.local/share/hero/sessions/<id>/assets/`; session-scoped only | Accepted |
| [ADR-081](ADR-C14-002-tui-multimodal-images.md#adr-081-unicode-mosaic-is-the-mandatory-preview-baseline-kittysixeliterm2-are-optional-and-off-by-default) | Unicode mosaic is mandatory preview baseline; Kitty/Sixel/iTerm2 are optional and off by default | Accepted |
| [ADR-082](ADR-C14-002-tui-multimodal-images.md#adr-082-any-image-file-written-by-a-tool-during-a-turn-is-exposed-as-a-model-output-asset-card) | Any image file written by a tool during a turn is exposed as a model output asset card | Accepted |
| [ADR-083](ADR-C15-001-loopback-findings-handoff.md#adr-083-findings-are-sqlite-operational-state-owned-by-the-go-scheduler) | Findings are SQLite operational state owned by the Go scheduler | Accepted |
| [ADR-084](ADR-C15-001-loopback-findings-handoff.md#adr-084-failed-stage-close-finding-persistence-and-loop-back-are-one-transaction) | Failed-stage close, finding persistence, and loop-back are one transaction | Accepted |
| [ADR-085](ADR-C15-001-loopback-findings-handoff.md#adr-085-implementation-assignments-union-openspec-tasks-with-actionable-findings) | Implementation assignments union OpenSpec tasks with actionable findings | Accepted |
| [ADR-086](ADR-C15-001-loopback-findings-handoff.md#adr-086-structured-agent-contracts-fail-closed-before-state-mutation) | Structured agent contracts fail closed before state mutation | Accepted |
| [ADR-087](ADR-C15-001-loopback-findings-handoff.md#adr-087-deferred-findings-become-durable-project-todos-with-a-reconciled-file-projection) | Deferred findings become durable project ToDos with a reconciled file projection | Accepted |
| [ADR-088](ADR-C15-001-loopback-findings-handoff.md#adr-088-escalation-triage-may-complete-a-cycle-with-a-deferred-work-disposition) | Escalation triage may complete a cycle with a deferred-work disposition | Accepted |
| [ADR-089](ADR-C15-001-loopback-findings-handoff.md#adr-089-research-adopts-pending-todos-manual-completion-is-pending-only) | Research adopts pending ToDos; manual completion is pending-only | Accepted |
| [ADR-090](ADR-C15-001-loopback-findings-handoff.md#adr-090-existing-status-and-event-surfaces-project-the-active-findings-flow) | Existing Status and Event surfaces project the active findings flow | Accepted |
| [ADR-091](ADR-C16-001-tui-session-history.md#adr-091-hero-session-identity-is-durable-and-distinct-from-native-harness-identity) | Hero session identity is durable and distinct from native harness identity | Proposed |
| [ADR-092](ADR-C16-001-tui-session-history.md#adr-092-a-session-aggregate-and-append-only-normalized-events-become-the-conversation-source-of-truth) | Session aggregate and append-only normalized events are the conversation source of truth | Proposed |
| [ADR-093](ADR-C16-001-tui-session-history.md#adr-093-conversation-lifecycle-belongs-in-a-focused-service-bubble-tea-remains-an-asynchronous-viewcontroller) | Conversation service owns lifecycle; Bubble Tea remains asynchronous | Proposed |
| [ADR-094](ADR-C16-001-tui-session-history.md#adr-094-database-leases-enforce-one-continuation-owner-per-session) | Database leases enforce one continuation owner per session | Proposed |
| [ADR-095](ADR-C16-001-tui-session-history.md#adr-095-native-resume-is-exact-incompatibility-creates-an-explicit-context-fork) | Native resume is exact; incompatibility creates an explicit context fork | Proposed |
...
**Decision**: Invoke every subagent (`backend_agent`, `frontend_agent`, `generic_agent`, `qa_agent`, `judge_agent`, `browser_ui_agent`, `end2end_qa_agent`, `context_agent`) via the IDE's Task tool, in a **fresh, isolated session** that does not inherit the orchestrator's chat history. **TUI amendment (ADR-054):** in Hero TUI, named **stage** agents are invoked by TUI-direct `HarnessAdapter.Execute` on the agent’s YAML harness+model pair (same isolation and file-pointer prompt as Task). Nested generic fan-out and named helpers such as `context_agent` launched *from* those agents remain Task inside the parent harness. Cursor IDE Runtime is unchanged and still uses Task for every subagent. Subagents receive **file pointers** (paths to `AGENTS.md`, `current-state.md`, the relevant SDD/tasks) instead of pasted content. The orchestrator absorbs back only the subagent's final structured output (its documented "Output Format" section), not its intermediate reasoning. The `orchestration_agent`'s own main session remains continuous across the whole cycle — only subagents are session-scoped per call.
