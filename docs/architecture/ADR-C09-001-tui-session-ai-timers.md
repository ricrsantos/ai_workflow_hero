...
Cycle session time must survive a TUI restart and must not include time after a
cycle is finished or archived. Free-chat time and AI time must not be persisted.
...
2. Add `cycles.session_duration_seconds` in SQLite schema v8. TUI ticks write
   ...
   newer value. A TUI restart resumes from that value and starts a new local
   segment, so closed-TUI time is excluded.
3. Start the cycle Session timer when `/hero-new` is dispatched. Stop and save
   it on a terminal cycle state; archive resets the displayed Session value.
   Starting free chat resets Session to zero and starts it on the first
   submitted prompt. Free-chat Session state is process-local.
