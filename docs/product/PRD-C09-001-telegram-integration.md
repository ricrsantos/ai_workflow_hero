...
- Telegram `/status` is TUI-owned and does not consume a harness turn. Every response includes `Agent state: working` while Chat has an active execution, live agent, or `/hero-start` preflight, and `Agent state: idle` otherwise. An idle response also returns `idle` with the selected free-chat model, the TUI `Session`, `AI wk`, `AI rp`, and context-window `used/max` values; the context usage is the occupancy of the session currently shown in Chat (last model-call prompt including cache, plus that turn's output), not a billed sum across turns. Ordinary freechat while a cycle is active uses the freechat session's occupancy, not the stage agent's. An active cycle returns its cycle title and current-stage state (without the cycle objective/summary) plus those same counters; an active free-chat turn returns `Waiting for harness` with those same counters. During an active turn, the response also lists each operating agent and its model; the ordinary Free Chat parent is named `harness`. Every Telegram request that starts or queues a harness turn also receives this status immediately.
- Telegram `/help` is daemon-owned and does not consume a harness turn. It works with or without a selected instance and returns a compact catalog of routing, control, cycle-config, permission, queue, and Hero slash commands available over Telegram.
- Telegram `/interrupt` is TUI-owned and does not consume a harness turn. When a Harness Execute or `/hero-start` preflight is active, it performs the same cancellation as Chat `Ctrl+C` and confirms the interruption request; when idle, it reports that there is no active process.
- Telegram `/kill` is TUI-owned and does not consume a harness turn. It is a last-resort remote shutdown of the selected TUI instance only (not the Telegram daemon): the IPC client goroutine best-effort acknowledges delivery, sends `Killing TUI.`, then force-kills the TUI process with `SIGKILL` so a wedged Bubble Tea Update loop cannot block it. There is no confirmation step.
- Harness-native permission requests (for example OpenCode tool access) are distinct from cycle-stage approval. The TUI keeps the local `y`/`n` prompt and, when Telegram is paired, forwards the request ID and a safe description; the remote user answers with `/hero-permission <id> allow` or `/hero-permission <id> deny`. Unknown or stale IDs are rejected without releasing another request.
- Project Settings persists `telegram.auto_report_minutes`: `0` disables automatic status reports and `1` through `300` sends the same non-idle status on that interval while Telegram is paired and connected. Automatic reporting never sends `idle`; only an explicit Telegram `/status` request may return `idle`. The boolean `telegram.always_send` defaults to `false`; when enabled, the TUI also forwards each completed harness turn response to the paired Telegram chat, including turns started locally in the TUI.
...
- After `/hero-new`, the TUI suggests `/hero-config`. The address-scoped `/hero-config` wizard guides title, objective, preferred language, scope, enabled stages, and human-approval choice for each enabled stage, followed by optional cycle-agent model review; `/hero-config-show` returns the current canonical configuration or the active draft. Model review is derived from the agents required by the enabled stages and selected scopes, so stale agent blocks outside the active scope are neither prompted nor shown. For every active named agent it reviews the parent harness/model/properties and then the nested subagent choice; a dedicated subagent model reuses the parent harness and selects its own model/properties. Selecting a new parent agent/model resets the nested subagent to `same_of_agent: true` until a dedicated model is explicitly chosen. The fallback model is reviewed once and has no subagent. All choices reuse the numbered Telegram `/model` flow and save to the workflow draft rather than free-chat `hero.json`.
- The cycle-configuration wizard keeps its draft in memory until an explicit save confirmation, validates it with the same Config rules, atomically updates the active cycle `workflow-config.yml`, and synchronizes the active cycle. `/hero-config cancel` discards the draft without changing the file.
...
- Cycle/stage lifecycle events produced by `hero` CLI child processes are sent through a private, per-TUI Unix socket inherited by the OpenCode serve process. The TUI forwards the same filtered lifecycle notifications to Telegram, including approval-required events. SQLite remains the durable audit source and is not polled for live delivery. Each event carries its owning project and the relay drops events whose project does not match the relay's own, so a process that merely inherits the endpoint (for example a test binary) cannot inject notifications; the TUI additionally ignores events for a cycle it does not own.
...
9. After `/hero-new`, `/hero-config` can edit and explicitly save a cycle draft from Telegram; `/hero-config-show` reads the canonical file; model selection covers only agents required by enabled stages/scopes, includes each active agent's nested subagent choice, uses the numbered Telegram `/model` interaction, and does not mutate the free-chat model.
10. `/interrupt` cancels in-flight Execute or `/hero-start` preflight on the selected TUI using the Chat `Ctrl+C` path; `/kill` force-kills that TUI process from the IPC client goroutine without stopping the daemon.
11. `/help` returns the Telegram command catalog without requiring `/select` and without forwarding a harness turn.
