...
- Telegram `/status` is TUI-owned and does not consume a harness turn. It returns `idle` with the selected free-chat model, the TUI `Session`, `AI wk`, `AI rp`, and context-window `used/max` values; the context usage is the latest completed Execute's normalized `input+output` approximation, not a cumulative sum across turns. An active cycle returns its cycle title and current-stage state (without the cycle objective/summary) plus those same counters; an active free-chat turn returns `Waiting for harness` with those same counters. During an active turn, the response also lists each operating agent and its model; the ordinary Free Chat parent is named `harness`. Every Telegram request that starts or queues a harness turn also receives this status immediately.
- Telegram `/interrupt` is TUI-owned and does not consume a harness turn. When a Harness Execute or `/hero-start` preflight is active, it performs the same cancellation as Chat `Esc` and confirms the interruption request; when idle, it reports that there is no active process.
