...
9. After `/hero-new`, `/hero-config` can edit and explicitly save a cycle draft from Telegram; `/hero-config-show` reads the canonical file; model selection uses the numbered Telegram `/model` interaction and does not mutate the free-chat model.
