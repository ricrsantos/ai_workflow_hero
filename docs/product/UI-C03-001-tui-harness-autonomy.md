...
The project navigation sidebar order is **Chat | Status | Artifacts | Costs | Events**, with **Config** appended only while an active cycle exists. Shortcuts follow the visible order (`alt+1` = Chat through `alt+5` = Events; `alt+6` = Config when visible); Ctrl aliases are unsupported. The navbar footer shows only `alt+1-5` or `alt+1-6` accordingly. TUI boot opens **Chat** (composer focused).

`Tab` and `Shift+Tab` switch focus between the active screen and the navbar. When the navbar is focused, Up/Down move a wrapping luminous background selection without changing the active-screen `>` marker; Enter opens the selected screen, and Tab returns focus to its content. A hidden narrow-terminal navbar cannot receive focus. Chat mode changes with `alt+m`, not Tab.
...
| Scroll hint | Directly under the transcript: property status labels and context bar (`█` filled / `░` empty) plus compact `used/max` (`180k/256k`) when the speaking model's `context_window` is in `models/*.yml`. Fill uses green below ~80%, yellow below ~95%, red at or above ~95% (UI.md §2.1). Omitted when the slug has no window. Used tokens are **window occupancy** for the session currently shown: the last model call's full prompt (uncached input plus cache read/write) plus that turn's output. Freechat and cycle-agent sessions are stored separately, so ordinary Chat during an active cycle does not inherit the stage agent's window. Adapters must not sum tool-loop steps into occupancy. When the harness omits last-call occupancy (including billed run totals without a per-call snapshot), occupancy is estimated as chars÷4 of that session's transcript. Cycle Costs remain a billed sum and are never the bar. `/new-chat` and a successful `/harness-reset` reset the displayed session. Keyboard scroll hints stay in the footer (`↑↓ scroll`). |
...
Esc focuses the navbar while a Harness Execute or `/hero-start` preflight is active; Ctrl+C takes priority as the interruption key.
