...
- **C14 multimodal tests**: fixture-only image inputs and outputs for the shared harness contract, session-store permissions and SHA-256 dedupe, magic-byte/header and dimension limits, retention cleanup, capability intersection/admission, Chat attachment chips and image-only submit across conversational modes, keyboard focus and open/copy/save/remove actions for asset cards and composer chips, and Codex/OpenCode/Claude/Cursor native/degraded/tool-written paths. No test invokes a real provider account or logs image bytes.
